...
impl<T:Clone> Vec2<T> {
	...
	pub fn splat(v:T)->Vec2<T> { Vec2{x:v.clone(),y:v.clone()}}
}

impl<T:Clone+Num> Vec2<T> {
	pub fn cross_to_scalar(&self,other:&Vec2<T>)->T {self.x*other.y-self.y*other.x}
	pub fn cross_z(&self,z:T)->Vec2<T> { Vec2::new(-self.y*z,self.x*z)}
	pub fn cross_one(&self)->Vec2<T> { Vec2::new(-self.y.clone(),self.x.clone())}
}
impl<T:Clone> Vec3<T> {
...
pub trait VecPermute<T:Clone+One+Zero> : VecAccessors<T> {
	fn xy(&self)->Vec2<T>	{ Vec2::new(self.x(),self.y())}
	fn yx(&self)->Vec2<T>	{ Vec2::new(self.y(),self.x())}
	fn xz(&self)->Vec2<T>	{ Vec2::new(self.x(),self.z())}
	fn yz(&self)->Vec2<T>	{ Vec2::new(self.y(),self.z())}
...
//	fn max(&self,b:&Vec2<T>)->Vec2<T>	{Vec2::new(zero(),zero())}
	...
	fn from_xyz(x:T,y:T,_:T)->Vec2<T>{Vec2::new(x.clone(),y.clone())}
}
impl<T:PartialOrd+Clone> VecCmp<T> for Vec2<T> {
	fn min(&self,b:&Vec2<T>)->Vec2<T>	{Vec2::new(min(self.x.clone(),b.x.clone()),min(self.y.clone(),b.y.clone()))}
	fn max(&self,b:&Vec2<T>)->Vec2<T>	{Vec2::new(max(self.x.clone(),b.x.clone()),max(self.y.clone(),b.y.clone()))}
...
impl<T:Float> Scale<T> for Vec2<T> {
	fn scale(&self,f:T)->Vec2<T>		{Vec2::new(self.x*f,self.y*f)}
}

impl<T:Float> SumElems<T> for Vec2<T> {
	fn sum_elems(&self)->T	{self.x+self.y}
}

impl<T:Float> Cross<T,Vec2<T>> for Vec2<T> {
	fn cross(&self,_:&Vec2<T>)->Vec2<T>{Vec2::new(zero::<T>(),zero::<T>())}
...
impl<T:Clone+PartialOrd> VecCmp<T> for Vec3<T> {
...
impl<T:Clone> Vec2<T> {
	fn to_tuple(&self)->(T,T) { (self.x.clone(),self.y.clone()) }
	fn from_tuple((x,y):(T,T))->Vec2<T> { Vec2{x:x.clone(),y:y.clone()} }
}
...
impl<T:Clone+PartialOrd> VecCmp<T> for Vec4<T> {
...
pub fn vec2<T:Clone>(x:T,y:T)->Vec2<T>{ Vec2::new(x,y) }
pub fn vec3<T:Clone>(x:T,y:T,z:T)->Vec3<T>{ Vec3::new(x,y,z) }
pub fn vec4<T:Clone>(x:T,y:T,z:T,w:T)->Vec4<T>{ Vec4::new(x,y,z,w) }
